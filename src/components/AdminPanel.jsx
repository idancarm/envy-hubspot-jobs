...
const AdminPanel = ({ services, bundles, onAdd, onEdit, onDelete, onReorder, onAddBundle, onEditBundle, onDeleteBundle, uiSettings, onUpdateSettings, onClose }) => {
    ...
    const handleServiceSubmit = (e) => {
        ...
        const payload = {
            ...
            monthlyPrice: Number(serviceForm.monthlyPrice || 0),
            ...
            <div className="flex flex-col lg:flex-row gap-8 h-full overflow-hidden">
                ...
                <div className="w-full lg:w-1/3 bg-gray-50/50 p-6 rounded-xl border border-slate-800 h-fit overflow-y-auto max-h-full">
                    ...
                    <div className="bg-white p-3 rounded-lg border border-gray-200 space-y-3">
                        <label className="block text-xs font-bold text-gray-700">Pricing Configuration</label>

                        <div>
                            <label className="block text-xs text-textMuted mb-1">Model</label>
                            <select
                            ...
                            >
                                ...
                                <option value="hybrid">Hybrid (Setup + Monthly)</option>
                                <option value="variable">Variable / Starting At</option>
                            </select>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs text-textMuted mb-1">
                                    {serviceForm.pricingModel === 'hybrid' ? 'Setup Fee ($)' : serviceForm.pricingModel === 'variable' ? 'Starting Price ($)' : 'Price ($)'}
                                </label>
                                <input
                                    type="number"
                            ...
                            {serviceForm.pricingModel === 'hybrid' && (
                                <div>
                                    <label className="block text-xs text-textMuted mb-1">Monthly Cost ($)</label>
                                    <input
                                        type="number"
